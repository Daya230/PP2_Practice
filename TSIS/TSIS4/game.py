...
def run_game(screen, username, settings):
    ...
    while running:
        ...
        # 8. Отрисовка
        screen.fill(BLACK)
        # Проверяем значение 'grid' из словаря settings, загруженного из JSON
        if settings.get("grid", False): 
            # Рисуем вертикальные линии через каждый шаг CELL
            for x in range(0, WIDTH, CELL):
                pygame.draw.line(screen, (40, 40, 40), (x, 0), (x, HEIGHT))
            # Рисуем горизонтальные линии
            for y in range(0, HEIGHT, CELL):
                pygame.draw.line(screen, (40, 40, 40), (0, y), (WIDTH, y))
        # Препятствия (отрисовываем серым, чтобы не путать с едой)
        for obs in obstacles:
            pygame.draw.rect(screen, (100, 100, 100), (obs.x * CELL, obs.y * CELL, CELL, CELL))

        # Еда (Красная) и Яд (Темно-красный)
        pygame.draw.rect(screen, RED, (food.x * CELL, food.y * CELL, CELL, CELL))
        pygame.draw.rect(screen, (139, 0, 0), (poison.x * CELL, poison.y * CELL, CELL, CELL))

        # Змейка (используем цвет из настроек и заполняем весь CELL)
        for part in snake.body:
            pygame.draw.rect(screen, snake.color, (part.x * CELL, part.y * CELL, CELL, CELL))
